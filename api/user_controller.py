...
@router.get("/{id}", summary="Получить Блюдо по идентификатору", response_model=UserResponse)
...
@router.get("", summary="Получить все Блюда", response_model=List[UserResponse])
...
@router.post("", summary="Создать Блюдо", response_model=UserResponse)
...
async def delete(id: str, db: AsyncIOMotorDatabase = Depends(get_db)):
    try:
        ...
        if result.deleted_count > 0:
            ...
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Блюдо не найден")
    except Exception as e:
        error_message = f"An error occurred: {str(e)}"
        log_error(f"Произошла ошибка: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=error_message)
...
@router.patch("/{id}", summary="Изменить данные о Блюде", response_model=UserResponse)
async def update(id: str, item: UserUpdate, db: AsyncIOMotorDatabase = Depends(get_db)):
    try:
        ...
        if updated_item is None:
            raise HTTPException(status_code=404, detail="Такого Блюда нет")
